...
func (key *Key) DeriveFromKeyPath(keyPath string) (*Key, error) {
	index, err := CheckKeyPath(keyPath)
	if err != nil {
		return nil, err
	}
	child, err := key.NewChildKey(index[0])
	if err != nil {
		return nil, err
	}
	for _, value := range index[1:] {
		child, err = child.NewChildKey(value)
		if err != nil {
			return nil, err
		}
	}
...
// 将HD显示的数字转换为uint32
func CheckKeyPath(keyPath string) ([]uint32, error) {
	var res []uint32
	explodeKetPath := strings.Split(keyPath, "/")
	if explodeKetPath[0] != "m" {
		return nil, errors.New("invalid first floor")
	}
	explodeKetPath = explodeKetPath[1:]
	for _, value := range explodeKetPath {
		isForced := strings.HasSuffix(value, "'")
		...
		if isForced {
			index32 = ParseHDNum(index32)
			if index32 < 2147483648 || index32 > 4294967295 {
				return nil, errors.New("invalid index number")
			}
		} else {
			if index32 > 2147483648 || index32 < 0 {
				return nil, errors.New("invalid index number")
			}
		}
		res = append(res, index32)
	}
	return res, nil
}
